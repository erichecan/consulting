...
export default async function TransportationPage() {
  ...
  const featuredCases = [
    { slug: "wholesale-tms-freight-savings", metric: "22%", metricLabel: "freight cost reduction" },
    { slug: "wholesale-distribution-network", metric: "$1.4M", metricLabel: "annual savings" },
  ];

  return (
    <main style={{ paddingTop: "68px" }}>
      {/* Hero */}
      <section style={{ backgroundColor: accent, color: "#fff", padding: "80px 32px 72px" }}>
        <div style={{ maxWidth: "1280px", margin: "0 auto" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "24px" }}>
            <Link href={l("/services")} style={{ fontSize: "13px", color: "rgba(255,255,255,0.55)", textDecoration: "none" }}>
              {t("breadcrumbParent")}
            </Link>
            <span style={{ color: "rgba(255,255,255,0.3)" }}>→</span>
            <span style={{ fontSize: "13px", color: "rgba(255,255,255,0.8)" }}>Transportation Management</span>
          </div>
          <div style={{ display: "inline-block", fontSize: "11px", fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", color: "rgba(255,255,255,0.75)", border: "1px solid rgba(255,255,255,0.25)", padding: "4px 10px", borderRadius: "4px", marginBottom: "20px" }}>
            {t("eyebrow")}
          </div>
          ...
          <p style={{ fontSize: "18px", color: "rgba(255,255,255,0.82)", maxWidth: "580px", lineHeight: 1.65, marginBottom: "36px" }}>
            {t("sub")}
          </p>
          <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
            <Link href={l("/contact")} style={{ display: "inline-block", backgroundColor: "#fff", color: accent, padding: "13px 28px", borderRadius: "6px", fontWeight: 600, fontSize: "14px", textDecoration: "none" }}>
              {t("ctaBook")}
            </Link>
            <Link href={l("/cases")} style={{ display: "inline-block", backgroundColor: "transparent", color: "#fff", padding: "13px 28px", borderRadius: "6px", fontWeight: 500, fontSize: "14px", textDecoration: "none", border: "1px solid rgba(255,255,255,0.4)" }}>
              {t("ctaCases")}
            </Link>
          </div>
        </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "0" }} className="steps-grid">
            {steps.map((step, i) => (
              <div key={i} style={{ padding: "32px 28px", borderRight: i < steps.length - 1 ? "1px solid var(--color-border)" : "none" }}>
                <div style={{ fontFamily: "var(--font-heading-en)", fontSize: "48px", fontWeight: 700, color: "rgba(3,105,161,0.07)", lineHeight: 1, marginBottom: "16px", letterSpacing: "-0.04em" }}>
                  {step.step}
                </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: "20px" }}>
            {featuredCases.map((fc) => (
              <Link key={fc.slug} href={l(`/cases/${fc.slug}`)} style={{ textDecoration: "none" }}>
                <article className="case-card" style={{ backgroundColor: "#fff", border: "1px solid var(--color-border)", borderRadius: "10px", padding: "24px", borderTop: `3px solid ${accent}`, cursor: "pointer", transition: "box-shadow 0.2s" }}>
      ...
      <section style={{ backgroundColor: accent, padding: "80px 32px", textAlign: "center" }}>
        <div style={{ maxWidth: "560px", margin: "0 auto" }}>
          ...
          <p style={{ fontSize: "16px", color: "rgba(255,255,255,0.82)", marginBottom: "36px" }}>
            {t("ctaSub")}
          </p>
          <Link href={l("/contact")} style={{ display: "inline-block", backgroundColor: "#fff", color: accent, padding: "14px 36px", borderRadius: "6px", fontWeight: 600, fontSize: "15px", textDecoration: "none" }}>
            {t("ctaBtn")}
          </Link>
        </div>
      </section>
